...
namespace MLearning.Touch.Views
{
	...
	public class MainView : MvxViewController, IMLODelegate
	{
		...
		UILabel name;
		UILabel lastname;
		LoadingOverlay mlosOverlay, commentsOverlay;
		...
		public override void ViewDidLoad()
		{
			...
			mlosOverlay = new LoadingOverlay (new RectangleF (400, 170, 500, 100));
			commentsOverlay = new LoadingOverlay (new RectangleF (400, 500, 500, 100));

			circlesTable = new UITableView (new RectangleF (0, 108, 248, 308));
			detailView = new UIView (new RectangleF (330, 0, 670, 768));


			//twice its size to avoid white background when panning fast
			rightView = new UIView (new RectangleF (684, 0, 340 * 2, 768));
		...
		private void addLeftBar ()
		{
			...
			{
				Frame = new RectangleF (0, 0, 82, 768),
				Colors = Constants.MainViewGradientColors
			};
			View.Layer.InsertSublayer (leftLayer, 0);

			//red logo
			Add (Constants.makeImageView (new RectangleF (16, 20, 50, 50), "iOS Resources/muro/logo.png"));

			var backButton = new UIButton (UIButtonType.Custom) {
				Frame = new RectangleF (16,90,50,50)
			};
			...
			var reloadButton = new UIButton (UIButtonType.Custom) {
				Frame = new RectangleF (16,160,50,50)
			};
		...
		private void loadUserInformation()
		{
			...
			userImageView = new UIImageView (new RectangleF (16, 671, 50, 50));
			userImageView.Layer.CornerRadius = 25f;
			userImageView.Layer.MasksToBounds = true;
			Add (userImageView);

			name = Constants.makeLabel (new RectangleF (16, 732, 50, 12), UIColor.White, UITextAlignment.Center, Font.Regular, 12);
			name.AdjustsFontSizeToFitWidth = true;
			Add (name);

			lastname = Constants.makeLabel (new RectangleF (16, 744, 50, 12), UIColor.White, UITextAlignment.Center, Font.Regular, 12);
		...
		{
			searchBackground = new UIView (new RectangleF (82, 0, 248, 768));
			searchBackground.BackgroundColor = Constants.SearchBarColor;

			var searchImage = new UIImageView (new RectangleF (24, 18, 192, 42));
			searchImage.Image = UIImage.FromFile ("iOS Resources/muro/searchback.png");
			searchBackground.Add (searchImage);

			searchText = Constants.makeTextField (new RectangleF (56, 24, 160, 30), UIColor.DarkGray, "Buscar", Font.Regular, 14);
			...
			var circulos = Constants.makeLabel (new RectangleF (24, 80, 220, 24), UIColor.Gray, UITextAlignment.Left, Font.Bold, 15);
			circulos.AdjustsFontSizeToFitWidth = true;
			circulos.Text = "CIRCULOS";
			searchBackground.Add(circulos);

			var personas = Constants.makeLabel (new RectangleF (24, 424, 220, 24), UIColor.Gray, UITextAlignment.Left, Font.Bold, 15);
		...
		{
			var peopleTable = new UITableView (new RectangleF (0, 454, 248, 302));
		...
		private void addDetails ()
		{
			...
			//linea1
			detailView.Add (Constants.makeImageView (new RectangleF (30, 69, 610, 1), "iOS Resources/muro/tareas/linea.png"));
			//linea2
			detailView.Add (Constants.makeImageView (new RectangleF (30, 316, 610, 1), "iOS Resources/muro/tareas/linea.png"));

			circleName = Constants.makeLabel (new RectangleF (48, 28, 483, 30), UIColor.Black, UITextAlignment.Left, Font.Light, 26);
			circleName.AdjustsFontSizeToFitWidth = true;
			circleName.Text = "Curso";
			detailView.Add (circleName);

			var comentarios = Constants.makeLabel (new RectangleF (48, 284, 140, 30), UIColor.Black, UITextAlignment.Left, Font.Light, 22);
			...
			unidades = Constants.makeLabel (new RectangleF (48, 82, 140, 30), UIColor.Black, UITextAlignment.Left, Font.Light, 22);
			...
			unidad = Constants.makeLabel (new RectangleF (48, 82, 110, 30), UIColor.Gray, UITextAlignment.Left, Font.Light, 22);
			unidad.Text = "Unidad  > ";
			detailView.Add (unidad);

			mloName = Constants.makeLabel (new RectangleF (162, 82, 180, 30), UIColor.Gray, UITextAlignment.Left, Font.Light, 22);
			mloName.AdjustsFontSizeToFitWidth = true;
			detailView.Add (mloName);

			mloOpen = new UIButton (UIButtonType.Custom);
			mloOpen.Frame = new RectangleF (300, 86, 74, 22);
			mloOpen.SetImage (UIImage.FromFile("iOS Resources/muro/tareas/btn_open.png"), UIControlState.Normal);
			detailView.Add (mloOpen);
			/*****/

			//commentBox
			detailView.Add (Constants.makeImageView (new RectangleF (30, 702, 610, 52), "iOS Resources/muro/tareas/cboard.png"));

			newCommentText = Constants.makeTextField (new RectangleF (46, 720, 548, 16), UIColor.Gray, "Comment", Font.Regular, 14);
			...
			commentButton.Frame = new RectangleF (600, 712, 32, 32);
		...
		void loadMLOS()
		{
			...
			mlosScroll = new UIScrollView (new RectangleF (38, 124, 620, 144));
			...
			mlosScroll.ContentSize = new SizeF (xPos, mlosScroll.Frame.Height);
		...
		void loadPosts()
		{
			...
			commentsScroll = new UIScrollView (new RectangleF (0, 334, 670, 340));
			...
			commentsScroll.ContentSize = new SizeF (commentsScroll.Frame.Width, totalHeight + 20);
		...
		private void addRightBar()
		{
			...
			rightLayer.Frame = new RectangleF (0, 0, rightView.Frame.Width, rightView.Frame.Height);
			...
			hideAndShowButton.Frame = new RectangleF (5, 377, 14, 14);
			...
			var tareas = Constants.makeLabel (new RectangleF (116, 30, 91, 24), UIColor.White, UITextAlignment.Center, Font.Regular, 20);
			tareas.AdjustsFontSizeToFitWidth = true;
			tareas.Text = "TAREAS";
			rightView.Add (tareas);

			//tareaImage
			rightView.Add (Constants.makeImageView (new RectangleF (210, 30, 22, 22), "iOS Resources/muro/tareas/btn_send.png"));
			//barraTarea
			rightView.Add (Constants.makeImageView (new RectangleF (24, 68, 316, 66),"iOS Resources/muro/tareas/barra_curso.png"));

			var curso = Constants.makeLabel (new RectangleF (70, 95, 55, 17), UIColor.White, UITextAlignment.Center, Font.Regular, 14);
			curso.AdjustsFontSizeToFitWidth = true;
			curso.Text = "CURSO";
			rightView.Add (curso);

			var unidad = Constants.makeLabel (new RectangleF (196, 95, 54, 17), UIColor.White, UITextAlignment.Center, Font.Regular, 14);
			unidad.AdjustsFontSizeToFitWidth = true;
			unidad.Text = "UNIDAD";
			rightView.Add (unidad);

			//circle1
			rightView.Add (Constants.makeImageView (new RectangleF (134, 84, 37, 36), "iOS Resources/muro/tareas/circulo.png"));
			//circle2
			rightView.Add (Constants.makeImageView (new RectangleF (260, 84, 37, 36), "iOS Resources/muro/tareas/circulo.png"));

			var unidadNumber = Constants.makeLabel (new RectangleF (268, 95, 21, 17), UIColor.White, UITextAlignment.Center, Font.Regular, 14);
			unidadNumber.AdjustsFontSizeToFitWidth = true;
			unidadNumber.Text = "?";
			rightView.Add (unidadNumber);

			var cursoNumber = Constants.makeLabel (new RectangleF (142, 95, 21, 17), UIColor.White, UITextAlignment.Center, Font.Regular, 14);
			...
			//Pendientes
			var pendientes = Constants.makeLabel (new RectangleF (42, 157, 102, 18), UIColor.White, UITextAlignment.Center, Font.Bold, 14);
			pendientes.Text = "PENDIENTES";
			rightView.Add (pendientes);
			//linea1
			rightView.Add (Constants.makeImageView (new RectangleF (40,187,284,1), "iOS Resources/muro/tareas/linea.png"));

			//Completadas
			var completadas = Constants.makeLabel (new RectangleF (42, 565, 109, 17), UIColor.White, UITextAlignment.Center, Font.Bold, 14);
			completadas.Text = "COMPLETADAS";
			rightView.Add (completadas);
			//linea2
			rightView.Add (Constants.makeImageView (new RectangleF (40,592,284,1), "iOS Resources/muro/tareas/linea.png"));
		...
		void animateRightView(float duration)
		{
			UIView.Animate (duration: duration,
				...
				animation: () => {
					var finalFrame = new RectangleF (684, 0, 340, 768);
		...
		void loadPendingQuizzes()
		{
			...
			pendingTable = new UIScrollView (new RectangleF (24, 192, 316, 358));
